...
var liveField=livespell.insitu.proxyDOM(id);if(!window.getSelection){var text=liveField.innerHTML;if(val.toLowerCase().indexOf("<p")==-1){val="<p>"+val+"</p>"}
...
try{n.setAttribute("class","livespell_textarea form-control col-md-6 "+t.className);}catch(e){}
...
n.isLiveSpellProxy=true;n.className="livespell_textarea form-control col-md-6";n.setAttribute("hasFocus",false)
...
livespell.events.add(window.document,"mousedown",livespell.context.hideIfNotinUse,false);livespell.events.add(window.document,"keydown",livespell.context.hide,false);livespell.userDict.load();livespell.context.hide();},allTextAreas:function(){var AF=[];var oTAreas=document.getElementsByTagName("textarea")
...
AF.push(Area.id);}}
return AF},allTextInputs:function(){var AF=[];var oTInputs=document.getElementsByTagName("input")
...
return AF},allEditors:function(){var AF=[];var odivs=document.getElementsByTagName("div")
...
var oIFrmaes=document.getElementsByTagName("iframe");for(var i=0;i<oIFrmaes.length;i++){var myFrame=oIFrmaes[i];oDoc=livespell.getIframeDocument(myFrame);if(oDoc){if(!myFrame.id){myFrame.id="livespell_rich_editor_id_"+i}
AF.push(myFrame.id);}}
