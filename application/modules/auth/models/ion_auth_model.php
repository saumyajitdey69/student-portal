...
class Ion_auth_model extends CI_Model
{
	...
	{
		$default_db=$this->load->database('default',TRUE,TRUE);
		...
		$this->trigger_events('extra_where');
		return $default_db->where('username', $username)
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		//check whether user is already there or not (Specific to our Student Portal)
		$query=$this->db->get_where($this->tables['student_data'],array('roll_number' =>$additional_data['roll_number'] ,'registration_number'=>$additional_data['registration_number'] ));
		if($query->num_rows()==1)
		{
			//student data exists
			$this->set_error('account_creation_duplicate_registration_no');
			return FALSE;
		}
		...
		//Add data into student_data table
		if(isset($id))
		{
